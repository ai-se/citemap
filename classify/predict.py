...
def ldade_linear_regression(*args):
  x_train = args[0]
  y_train = args[1]
  x_test = args[2]
  y_test = args[3]
...
def vectorized_linear_regression(*args):
  x_train = args[0]
  y_train = args[1]
  x_test = args[2]
  y_test = args[3]
...
def vectorized_decision_tree(*args):
  x_train = args[0]
  y_train = args[1]
  x_test = args[2]
  y_test = args[3]
...
def predict_conference(predictors, n_folds=10, n_topics=N_TOPICS, alpha=None, beta=None, n_iter=100,
  ...
  metrics_map = {predictor.__name__: [] for predictor in predictors}
  index = 0
  for train_indices, test_indices in skf.split(papers, groups):
    ...
    x_train, y_train, x_test, y_test = train_papers, groups[train_indices], test_papers, groups[test_indices]
    for predictor in predictors:
      print(predictor.__name__)
      predicted, metrics = predictor(x_train, y_train, x_test, y_test)
      metrics_map[predictor.__name__].append(metrics)
  for predictor in predictors:
    print (predictor.__name__)
    measures = Metrics.avg_score(metrics_map[predictor.__name__])
    print (measures)


predict_conference([vectorized_decision_tree, vectorized_linear_regression, ldade_decision_tree, ldade_linear_regression])
